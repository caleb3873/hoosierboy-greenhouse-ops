...
function houseStats(house) {
  ...
  house.zones.forEach(z => {
    if (z.type !== "bench") {
      capsPerType[z.type] = (capsPerType[z.type] || 0) + (z.items || []).reduce((s, r) => s + (Number(r.capacityPerRow) || 0), 0);
      linesPerType[z.type] = (linesPerType[z.type] || 0) + (z.items || []).length;
    }
  });
  return { benches: benches.length, sqFt, occupied, heated, capsPerType, linesPerType, available: benches.length - occupied };
}
    ...
    <div style={{ background: "#fff", borderRadius: 16, border: "1.5px solid #e0ead8", overflow: "hidden" }}>
      ...
      <div style={{ padding: "22px 24px" }}>
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 22 }}>
          ...
          <button onClick={() => { if (!form.name.trim()) return; onSave({ ...form, id: form.id || uid(), tempTier: form.tempTier || null, houseType: form.houseType || null }); }} style={{ flex: 1, background: form.name.trim() ? "#7fb069" : "#c8d8c0", color: "#fff", border: "none", borderRadius: 10, padding: 12, fontWeight: 700, fontSize: 15, cursor: form.name.trim() ? "pointer" : "not-allowed", fontFamily: "inherit" }}>{initial ? "Save Changes" : "Create House"}</button>
      ...
      <div style={{ padding: "16px 20px", display: "flex", gap: 14, alignItems: "flex-start" }}>
        <div style={{ flex: 1, minWidth: 0, cursor: "pointer" }} onClick={() => setExpanded(e => !e)}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            {Object.entries(st.linesPerType || {}).map(([t, count]) => count > 0 && (
              <Pill key={t} label={`${ztc(t).icon} ${t === "hanging" ? "Basket Lines" : t === "lowbasket" ? "Low Planter Lines" : "Rack Rows"}`}
                value={`${count}${st.capsPerType[t] > 0 ? ` · ${st.capsPerType[t].toLocaleString()} pos.` : ""}`}
                color={ztc(t).color} />
            ))}
          </div>
        </div>
...
function BayEditor({ bays, onChange }) {
  ...
  const dup = (idx) => { const copy = { ...bays[idx], id: uid(), number: `Bay ${bays.length + 1}` }; const next = [...bays]; next.splice(idx + 1, 0, copy); onChange(next); setEditIdx(idx + 1); };
...
function Overview({ houses, pads, locFilter }) {
  ...
  const totBasketLines  = filtH.reduce((s, h) => s + (h.zones || []).filter(z => z.type === "hanging").reduce((a, z) => a + (z.items || []).length, 0), 0);
  const totPlanterLines = filtH.reduce((s, h) => s + (h.zones || []).filter(z => z.type === "lowbasket").reduce((a, z) => a + (z.items || []).length, 0), 0);
    ...
    <div>
      ...
      <div style={{ background: "#fff", borderRadius: 14, border: "1.5px solid #e0ead8", padding: "18px 24px", marginBottom: 24 }}>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          ...
          {totBasketLines > 0 && <Pill label="🧺 Basket Lines" value={totBasketLines} color="#4a90d9" />}
          {totPlanterLines > 0 && <Pill label="🪴 Planter Lines" value={totPlanterLines} color="#8e44ad" />}
                ...
                {filtH.map(house => {
                  ...
                  return (
                    <div key={house.id} onClick={() => setSelected(isSelected ? null : { type: "house", id: house.id })}
                      ...
                      {Object.entries(st.linesPerType || {}).map(([t, count]) => count > 0 && (
                        <div key={t} style={{ fontSize: 11, color: ztc(t).color, marginTop: 3 }}>
                          {ztc(t).icon} {count} {t === "hanging" ? "basket line" : t === "lowbasket" ? "low planter line" : "rack row"}{count !== 1 ? "s" : ""}
                          {st.capsPerType[t] > 0 ? ` · ${st.capsPerType[t].toLocaleString()} positions` : ""}
                        </div>
                      ))}
