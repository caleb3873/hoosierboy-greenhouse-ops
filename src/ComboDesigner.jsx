                  ...
                  {form.hasTray && (
                    <div style={{ background: "#f0f5ff", border: "1.5px solid #c0d0f0", borderRadius: 10, padding: 14 }}>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                        ...
                        <div><FL c="Cost per Carrier Tray ($)" /><input type="number" step="0.001" style={{ ...IS(focus === "tcost"), background: "#fff" }} value={form.trayCost || ""} onChange={e => upd("trayCost", e.target.value)} onFocus={() => setFocus("tcost")} onBlur={() => setFocus(null)} placeholder="e.g. 0.45" /></div>
                        <div><FL c="Pots per Carrier Tray" /><input type="number" style={{ ...IS(focus === "tpc"), background: "#fff" }} value={form.traysPerCase || ""} onChange={e => upd("traysPerCase", e.target.value)} onFocus={() => setFocus("tpc")} onBlur={() => setFocus(null)} placeholder="e.g. 18, 24" /></div>
                      </div>
                      {form.trayCost && form.traysPerCase && <div style={{ marginTop: 10, fontSize: 12, color: "#3a4a80", fontWeight: 700 }}>Cost per pot position: ${(Number(form.trayCost) / (Number(form.traysPerCase) || 1)).toFixed(4)}</div>}
                      {form.trayCost && !form.traysPerCase && <div style={{ marginTop: 10, fontSize: 12, color: "#c8791a", fontWeight: 700 }}>⚠ Enter pots per tray to see cost per pot position</div>}
                    </div>
                  )}
                </div>
              )}
