      ...
      {adding
        ...
        : (
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            ...
            {items.filter(i => !i.scheduled_crop_id && i.container_id && +i.target_units > 0).length > 0 &&
              <button onClick={onConvert} title="creates plan rows for every unconverted item with a size and target — works from any status"
                style={{ padding: "7px 13px", borderRadius: 8, border: "none", background: C.dark, color: "#c8e6b8", fontSize: 12.5, fontWeight: 800, cursor: "pointer", fontFamily: "inherit" }}>→ Create plan items ({items.filter(i => !i.scheduled_crop_id && i.container_id && +i.target_units > 0).length})</button>}
