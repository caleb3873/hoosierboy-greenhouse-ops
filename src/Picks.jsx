...
export default function PicksViewer({ token }) {
  ...
  const perSheet = useMemo(() => {
    ...
    return state.sheets.map(s => {
      ...
      // Inside a breeder block: straight A→Z by variety name (Caleb 9/13). Series still cluster because the
      // name starts with the series; doubles interleave with their singles, and the Form filter isolates them.
      secs.forEach(sec => sec.items.sort((a, b) => a.name.localeCompare(b.name, "en", { numeric: true })));
