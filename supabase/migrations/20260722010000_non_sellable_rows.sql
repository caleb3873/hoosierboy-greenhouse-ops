-- Some plan rows are INPUTS, not products.
--
-- `38 CELL DRACAENA INDIVISA SPIKES` and `72 CELL BIG TWISTER` are propagation
-- trays — grown, then transplanted into containers, never sold on their own
-- (confirmed by Caleb for the spikes). They were nonetheless treated as sellable
-- items: both got B2B production_items and draft catalog profiles, and both
-- produced nonsense ready dates (the spikes "ready" wk48/2026 for a spring crop)
-- because a tray has no meaningful customer-ready week.
--
-- The plan already distinguishes combo components via is_combo_component, but
-- that requires a combo_parent_id and these feed many containers rather than one.
-- So: an explicit `sellable` flag, which the B2B reconcile honours alongside
-- is_combo_component. Anything grown as an input can be marked once and stays
-- out of the catalog permanently.

alter table scheduled_crops add column if not exists sellable boolean not null default true;
comment on column scheduled_crops.sellable is
  'False for rows grown as inputs (propagation trays, material transplanted into containers). Excluded from the B2B catalog and from sell-through analysis.';

-- the two known tray rows
update scheduled_crops sc set sellable = false
from production_plans p
where p.id = sc.plan_id and p.name = 'Spring 2027'
  and sc.item_name ~* '^[0-9]+ ?CELL';

-- remove the catalog entries they should never have had.
-- Verified first: 0 order lines, 0 hot-list items, 0 published profiles,
-- 0 inventory events. Profiles go before items (the FK is delete-RESTRICT).
delete from product_profiles pp
where pp.production_item_id in (
  select distinct sc.production_item_id from scheduled_crops sc
  join production_plans p on p.id = sc.plan_id
  where p.name = 'Spring 2027' and sc.sellable = false and sc.production_item_id is not null);

delete from production_item_groups g
where g.production_item_id in (
  select distinct sc.production_item_id from scheduled_crops sc
  join production_plans p on p.id = sc.plan_id
  where p.name = 'Spring 2027' and sc.sellable = false and sc.production_item_id is not null);

with doomed as (
  select distinct sc.production_item_id as pid from scheduled_crops sc
  join production_plans p on p.id = sc.plan_id
  where p.name = 'Spring 2027' and sc.sellable = false and sc.production_item_id is not null
)
update scheduled_crops sc set production_item_id = null
from doomed d where sc.production_item_id = d.pid;

delete from production_items pi
where pi.plan_id = (select id from production_plans where name = 'Spring 2027')
  and not exists (select 1 from scheduled_crops sc where sc.production_item_id = pi.id);

-- ready dates are meaningless for an input row
update scheduled_crops sc set ready_week = null, ready_year = null, ready_source = null
from production_plans p
where p.id = sc.plan_id and p.name = 'Spring 2027' and sc.sellable = false;

-- Teach the reconcile to skip them, so they never come back on the next tick.
-- This is the LIVE function definition with `and coalesce(sc.sellable, true)`
-- added to every scheduled_crops scan that decides what becomes a catalog item.
...
begin
  ...
  where sc.plan_id = p_plan and not sc.is_combo_component and coalesce(sc.sellable, true)
  ...
  with base as (
    ...
    from scheduled_crops sc where sc.plan_id = p_plan and not sc.is_combo_component and coalesce(sc.sellable, true)
  ),
  grains as (
  ...
  with l1 as (
    ...
    from production_items pi
    where sc.plan_id = p_plan and not sc.is_combo_component and coalesce(sc.sellable, true) and sc.production_item_id is null
  ...
  where not sc.is_combo_component and coalesce(sc.sellable, true)
