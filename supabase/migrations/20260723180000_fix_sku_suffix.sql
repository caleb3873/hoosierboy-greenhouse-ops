-- Fix the reconcile SKU-suffix bug found during the Aida/Moni work: stripping
-- ALL non-digits reads W26-0001 as 260001 (S27-0001 as 270001), so the first
-- new grain on any plan collides. Take the digits AFTER the last dash instead.
-- Live definition with only that expression changed.
...
begin
  ...
  select coalesce(max(nullif(regexp_replace(sku, '^.*-', ''), '')::int), 0)
