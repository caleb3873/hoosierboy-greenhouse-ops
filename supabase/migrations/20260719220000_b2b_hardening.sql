-- B2B M16: hardening.
-- 1. Money tables → staff-only (floor codes are anon and must not read customer pricing/orders).
do $$ declare t text;
begin
  foreach t in array array['price_levels','customer_item_prices','level_item_prices','global_price_breaks',
                           'product_grade_prices','customer_orders','customer_order_lines','customer_order_events',
                           'customer_notes','customer_recommendations','hot_lists','hot_list_items'] loop
...
-- 2. Reconcile v2: also RELINK rows whose variety/container was EDITED after linking
--    (previously only null links were filled — an edited row kept pointing at the wrong item).
...
-- 3. Sensitive views must respect the CALLER's RLS (Postgres views default to owner
--    privileges, which would leak reservations/summaries to anon once data exists).
--    v_item_availability / v_item_trends stay default — they're the intended public reads.
alter view v_customer_reservations set (security_invoker = true);
alter view v_customer_season_summary set (security_invoker = true);
alter view v_hot_list_health set (security_invoker = true);
