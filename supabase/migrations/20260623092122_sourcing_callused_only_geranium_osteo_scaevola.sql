...
where season = '2026-2027' and landed is not null and landed > 0
  and form_class <> 'urc_autostix'
  -- geranium / osteospermum / scaevola are only brought in callused → show callused pricing only
...
with vb as (
  ...
  from broker_prices
  where season='2026-2027' and landed > 0 and form_class <> 'urc_autostix'
    and not (split_part(variety_key,' ',1) in ('geranium','osteospermum','scaevola') and form_class <> 'callused')
